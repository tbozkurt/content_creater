...
function AddPlayer(obj){
	...
	var conf = {occMode:false, div:"", divCSS:{}, src:"", width:"100%", autoStart:false, fullScreen:false, skin:true, videoCapture:false, vsMode:false, vsLevel:null, vkaMode:false, keyboardEvent:false, hype:{mode:false, height:720}, fullScreenFNC:null, endFNC:undefined, watchedFNC:undefined, metaDataFNC:undefined, currentMediaID:0, lang:"tr", showSpeed:false, timeRange:null};
	...
	function getRangeStart(){
		return methods.timeRange && methods.timeRange.minTime !== null ? methods.timeRange.minTime : 0;
	}

	function getRangeEnd(){
		if(!methods.timeRange || methods.timeRange.maxTime === null){
			return durTime;
		}

		return durTime ? Math.min(methods.timeRange.maxTime, durTime) : methods.timeRange.maxTime;
	}

	function getRangeDuration(){
		return Math.max(0, getRangeEnd() - getRangeStart());
	}

	function getRangeCurrentTime(){
		var rangeTime = curTime - getRangeStart();
		var rangeDuration = getRangeDuration();

		if(rangeTime < 0){
			return 0;
		}

		if(rangeDuration && rangeTime > rangeDuration){
	...
	function updateSliderAndTimeFNC(watch){
		...
		if(methods.globalPlay){
			if(watch){
				...
				methods.accessPercent = parseInt((watchTime / (getRangeDuration() || durTime))*100);
		...
		var rangeDuration = getRangeDuration();
		var math = parseInt((rangeDuration ? (getRangeCurrentTime()/rangeDuration) : 0)*100)+"%";
		if(!sliderCirclePress){
			sliderFront.css("width", math);
			sliderCircle.css("left", (sliderFront.width()-7));
		}

		methods.curTime = digit(parseInt(getRangeCurrentTime()));
		methods.durTime = digit(parseInt(rangeDuration || durTime));
	...
	function gotoCircle(e){
		...
		goes = Math.ceil(goes * parseInt(rangeDuration || durTime));
	...
	function gotoTime(e){
		if(sliderCirclePress && methods.firstPlay){
			...
			var rangeDuration = getRangeDuration();
			var goes = (localX / sliderWidth);
			goes = getRangeStart() + Math.ceil(goes * (rangeDuration || durTime));
	...
	function checkBuffer() {
		...
		if (videoBuffer) {
			for (var i=0; i<videoBuffer.length; i++) {
				...
				var rangeDuration = getRangeDuration() || video.duration;
				var bufferStart = Math.max(sendBuffer[i][0], rangeStart);
				var bufferEnd = Math.min(sendBuffer[i][1], getRangeEnd() || video.duration);
				startBuffer = (bufferStart - rangeStart) / rangeDuration * sliderWidth;
				endBuffer = (bufferEnd - rangeStart) / rangeDuration * sliderWidth;
